...
class UI:
    ...
    def display_filters(self):
        ...
        with col5:
            self.selected_win_names = st.multiselect('晋级品种名称:', test_available_names, default=test_available_names[1])
        ...
        # 按首字母进行排序
        ck_name_list =sorted(result_list)
    ...
    def display_charts(self):
        ...
        for (year, EntryBookPrj, trial_type), df_group in grouped_df:
            ...
            fig = go.Figure()
            if len(self.selected_win_names) >1  and self.selected_win_names[0]=='请选择':
            ...
            if value_name !='' or value_name=='CKmean':
                ...
                win_test_group = test_group[(test_group['CK']==value_name)&(test_group['Varnam'].isin(self.selected_win_names if len(self.selected_win_names)!=0 and self.selected_win_names[0]!='请选择' else []))]  # 从测试品种中选择晋级品种
