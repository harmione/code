...
class Plotter:
    ...
    def plot(self):
        ...
        for trait_name in selected_trait_column_list:
            ...
            # 将Nan换为空
            final_trait_summary_df = final_trait_summary_df.fillna('')
            #保留1位小数
           ...
           # style_df = final_trait_summary_df.style.apply(self.highlight_index, axis=1)
            ...
            # # 初始化session state中的复选框状态
            # if 'checkbox_states' not in st.session_state:
            #     st.session_state.checkbox_states = {id: False for id in final_trait_summary_df.index}
            # # 创建列布局
            # col1, col2 = st.columns([1, 4])
            # with col1:
            #     # 在第一列中为每一行创建复选框
            #     for index, row in final_trait_summary_df.iterrows():
            #         checkbox_key = f"checkbox_{row.index}"
            #         checked = st.checkbox(f'Hide ID {row.index}', key=checkbox_key)
            #         st.session_state.checkbox_states[row.index] = checked
            #
            # with col2:
            #     # 根据复选框的状态过滤DataFrame
            #     filtered_df = final_trait_summary_df[~final_trait_summary_df.index.isin([id for id, hide in st.session_state.checkbox_states.items() if hide])]
            #
            #     # 显示过滤后的DataFrame
            #     st.write(filtered_df)
            # 展示， height手动设置
            st.dataframe(final_trait_summary_df,height=700)
