...
class Plot:
    ...
    def get_dropdown_menu_bar(self):
        ...
        with column_list[1]:
            selected_target_name_list = st.multiselect(
                ...
                default=self.phenoTable.get_sample_name_list(selected_AOAname)[1:7]  # 默认
            )
    ...
    def __data_preprocessing(self, sample_data_df, ck_data_df, caculate_method):
        ...
        sample_bookname_set, ck_bookname_set = set(sample_data_df["Location_TD"]), set(
            ck_data_df["Location_TD"])  # 再仔细检查一下
        bookname_set = None
    ...
    def __single_sample_num_summary_caculate(self, origin_single_sample_data_df, origin_ck_data_df,
        ...
        for trait_column_name in trait_column_name_list:
            ...
            single_sample_data_df = origin_single_sample_data_df[
                ~pd.isna(origin_single_sample_data_df[trait_column_name])]
            ...
            trait_column_name = self.trait_column_name_to_chinese_name_dict[trait_column_name].split("(")[
                0]  # 切片只要性状名称 不需要所为哪一类别
    ...
    def __single_sample_grade_summary_caculate(self, origin_single_sample_data_df, origin_ck_data_df,
                                               trait_column_name_list,
    ...
    def __single_sample_percent_summary_caculate(self, origin_single_sample_data_df, origin_ck_data_df,
                                                 trait_column_name_list,
                                                 selected_year_list, selected_AOAname):
        single_sample_summary_df = pd.DataFrame({
            '品种综合评分': [np.nan],
        ...
        for trait_column_name in trait_column_name_list:  # 遍历所有的性状列
            single_sample_plot_count = len(origin_single_sample_data_df)  # 统计样本种植的点的数量
            single_sample_data_df = origin_single_sample_data_df[
                ~pd.isna(origin_single_sample_data_df[trait_column_name])]  # 进行过滤 样本集中移除包含缺失值的所有行【因为性状是列，样本及对照数据按行来】
            ck_data_df = origin_ck_data_df[~pd.isna(origin_ck_data_df[trait_column_name])]  # 移除 对照集中为缺失值的所有行
            # 获取压力点次（或发生点次）
            pressure_bookname_num = self.phenoTable.get_percent_pressure_bookname_num(selected_year_list,
                                                                                      selected_AOAname,
                                                                                      trait_column_name)
            single_sample_trait_count = len(single_sample_data_df['Location_TD'].unique())  # 统计有该性状的点的数量
    ...
    def __get_normalized_trait_data_df(self, summary_data_df, trait_column_name_list):
        # 将数据统计信息进行标准化（不区分哪种表）
        for trait_column_name in trait_column_name_list:
            if '%' in self.trait_column_name_to_chinese_name_dict[trait_column_name] and '水分' not in \
                    self.trait_column_name_to_chinese_name_dict[trait_column_name]:
                flag = 1
            else:
                flag = 0
            trait_column_name = \
            self.trait_column_name_to_chinese_name_dict[trait_column_name].split("(")[0].split('比例')[0]
    ...
    def __get_sample_score_data_df(self, summary_data_df: pd.DataFrame, trait_column_name_list) -> pd.DataFrame:
        ...
        for trait_column_name in trait_column_name_list:
            chinese_trait_column_name = \
            self.trait_column_name_to_chinese_name_dict[trait_column_name].split("(")[0].split("比例")[
                0]  # split将字符串分割成列表
        ...
        def score_caculate(row: pd.DataFrame):
            ...
            for trait_column_name in not_null_trait_column_name_list:
                ...
                score = score + float(row[chinese_trait_column_name + "标准化"]) * self.trait_weight_coefficient_dict[
                    trait_column_name]  # 标准化值* 特定权重系数
            return score

        summary_data_df["品种综合评分"] = summary_data_df.apply(lambda row: score_caculate(row),
                                                                axis=1)  # apply方法作用于每一行，并将结果存储在 品种综合评分列里
        summary_data_df = summary_data_df.applymap(self.format_float)
        return summary_data_df
    ...
    def num_trait_summary(self, sample_data_df, ck_data_df, trait_column_name_list, selected_caculate_method):
        ...
        sample_data_df, ck_data_df = self.__data_preprocessing(sample_data_df, ck_data_df,
                                                               selected_caculate_method)  # 交并选择，获取对应的数据集
    ...
    def __get_grouped_trait_column_list(self, selected_trait_column_list):
        ...
        for selected_trait_column_name in selected_trait_column_list:
            if "%" in selected_trait_column_name and "水分" not in selected_trait_column_name:  # 筛选出只包含% 不包含水分的列
                percent_trait_column_list.append(
                    self.chinese_name_to_trait_column_name_dict[selected_trait_column_name])
...
def main():
    # st.set_page_config(layout='wide')   # 页面设置宽屏布局    调用该python文件时，注释掉，该文件单独运行时，取消注释
    ck_query_statement = """
