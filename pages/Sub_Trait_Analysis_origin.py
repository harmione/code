...
class Plot:
   ...
   # 获取数值型的 统计信息   不包括水分的
    def get_num_trait_summary(self, year, AOAname_list, sample_data_df, ck_data_df, trait_column_name):
        ...
        ck_bookname_set = pd.unique(ck_data_df[ck_data_df['VarNam'] == ck_name]["Location_TD"])   # 种的地点
        ...
        for sample_name in sample_name_set:
            ...
            sample_bookname_set = pd.unique(sample_data_df[sample_data_df['VarNam'] == sample_name]["Location_TD"])  # 样本测试点的集合
    ...
    def get_mst_trait_summary(self, year, AOAname_list, sample_data_df, ck_data_df, trait_column_name):
        ...
        ck_bookname_set = pd.unique(ck_data_df[ck_data_df['VarNam'] == ck_name]["Location_TD"])
        num_trait_summary_df = pd.DataFrame()
        aoa_name = ["" + f"{item} " for item in AOAname_list][0]
        for sample_name in sample_name_set:
            single_sample_num_trait_summary_dict = {}
            sample_bookname_set = pd.unique(sample_data_df[sample_data_df['VarNam'] == sample_name]["Location_TD"])
    ...
    def kertpct_color_cells(self,row):   #霉变粒率病 东华北的 均值列
        colors = []
        for val in row:
            if 0.0 < val <= 0.5:
                colors.append("background-color:#228B22")  # 高抗
        ...
        for trait_column_name in selected_trait_column_list:
            ...
            if "YLD" in trait_column_name:
                ...
                if "霉变粒率比例(%)" in self.trait_column_name_to_chinese_name_dict[trait_column_name]:
                    style_df = style_df.apply(self.kertpct_color_cells, subset=['目标品种均值[%制]'])
