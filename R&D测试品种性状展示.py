...
# @Time : 20241211
#  :harmione
# @FileName: R&D测试品种性状展示2024_new.py
# @Software: PyCharm
...
class Plotter:
    ...
    def get_ck_name_list(self, AOAname, target_name_list):
        ck_name_list = list(filter(None, pd.unique(self.data_df[(self.data_df["EntryBookPrj"]==AOAname) &(self.data_df["Varnam"].isin(target_name_list if isinstance(target_name_list, list) else [target_name_list]))
                                                        ]["CK"]).tolist()))
    ...
    def get_use_data_df(self,AOAname,name_list, trait_name,years,types):
    ...
    def get_dropdown_menu_bar(self):
        ...
        column_list = st.columns((3,1,1,1))  # 设置页面布局的宽度比例
        with column_list[0]:
        ...
        data_type_list = ['请选择','性状绝对值','性状相对值']
        selected_data_type = data_type_list[0]
        with column_list[1]:
        ...
        column_list = st.columns((1,3,2))  # 设置页面布局的宽度比例
        # 选择试验类型
        ...
        with column_list[1]:
            selected_tar_name_list = st.multiselect(
                ...
                default = self.get_var_name_list(selected_AOAname)[0:3]
            )

        self.ck_name_list = self.get_ck_name_list(selected_AOAname,selected_tar_name_list)
        with column_list[2]:
        ...
        return (selected_data_type,selected_years,selected_AOAname,selected_types,
    ...
    def plot(self):
        self.__get_title(title_content="R&D测试品种性状展示_(品种多选)")
        selected_data_type,selected_years, selected_AOAname, selected_types,selected_ck_name, selected_target_name, selected_trait_list = self.get_dropdown_menu_bar()
        for trait_name in selected_trait_list:
            ...
            if trait_name in ["YLD14", "MST", "EHT", "PHT"]:
                if selected_data_type == "性状相对值":
                    ...
                    flag_type = 3
            #根据选择的条件，筛选数据集
            sample_data_df = self.get_use_data_df(selected_AOAname,selected_target_name, trait_name,selected_years,selected_types)
            ck_data_df = self.get_use_data_df(selected_AOAname,self.ck_name_list, trait_name,selected_years,selected_types)   # 获取的是几个ck所有的名字的整体数据
            ...
            if "All" in BookName_list and BookName_list[0]!="All":
                BookName_list.remove("All")
                BookName_list.insert(0,"All")
            total_data_rows = []
            for bookname in BookName_list:
                ...
                for tar in selected_target_name:
                    try:
                        trait_value_dict[tar] = pd.unique(sample_data_df[(sample_data_df["BookName"] ==bookname)&(sample_data_df["Varnam"] == tar) & (sample_data_df["CK"] == selected_ck_name)][trait_name])[0] # 该地点的目标品种的性状值
                    except:
                        print(bookname +"没有目标样本"+tar+"的数据")
                        trait_value_dict[tar] = 0.0
            ...
            if flag_type ==3:
                text_wold = trait_name.split('(')[0] +'绝对值'
            elif flag_type ==2:
                text_wold = trait_name.split('(')[0] + '绝对值'
            elif flag_type ==1:
                text_wold = trait_name.split('(')[0] + '相对值'
            fig = px.bar(
                total_data_df,
                x='bookname',
                y=color_list,
            ...
            fig.add_shape(
                type="line",
                x0=-0.5, x1=len(total_data_df.iloc[:]),  # 线的起始和结束x坐标，-0.5和len(x_data)-0.5确保线覆盖整个x轴范围
                y0=100, y1=100,  # 线的y坐标
                line=dict(
                    color="#38B0DE",  # 线的颜色
                    width=3,  # 线宽
...
def main():
    ...
    query_statement = """
                   ...
                   select 
                       ...
                       loc."LocationSelf"
                   from 
                       "test"."ANOVA.Trial-P1P2.EstimatedValue.EntryBookPrj" p 
                   left join 
